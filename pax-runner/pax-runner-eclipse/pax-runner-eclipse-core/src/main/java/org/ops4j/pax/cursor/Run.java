...
public class Run
{
    ...
    Map<BundleActivator, BundleContext> contexts = new HashMap<BundleActivator, BundleContext>();
...
//    }
     ...
     */
    public void main( final JavaRunner runner, final String... args )
    {
    ...
    BundleContext createActivator( final String bundleName, final String activatorClazz, final Context context )
    {
        try
        {
            ...
            contexts.put(activator, bundleContext);
    ...
    public void stop() {
        LOGGER.debug( "Stopping all pax bundles" );
        Collections.reverse(activators);
        for (BundleActivator a : activators) {
            LOGGER.debug( "Stopping " + a.getClass().getName());
        	try {
        		a.stop(contexts.get(a));
        	} catch (Exception e) {
        		e.printStackTrace();
        	}
        }
